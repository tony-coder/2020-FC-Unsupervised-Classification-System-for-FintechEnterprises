...
import time
import string
import core.MachineLearning as ML
...
def draw_bar_pic(bar_matrix, pic_doc, module_name, cols):
    ...
    for i in tqdm.tqdm(range(inner_num)):
        ...
        plt.title("No."+str(i)+" cluster",fontsize=20)
...
class Epoch:
    def __init__(self):
        ...
        self.weight_dict = get_json(os.getcwd()+"\\weight.json")
    ...
    def merge_mark(self):
        ...
        model_path = self.model_doc + module_name + "\\ent_inner_C15.m"
        a = time.time()
        if self.mode == self.TRAIN:
            inner_labels = ML.KM(data_matrix=data_matrix, C=15, is_ascend=False,
                                 model_path="new_model_doc\\"+ module_name + "\\ent_inner_C15.m")  # 内部聚簇
        else:
...
#
# 用默认模型预测
# epoch.predict_use_default_model(csv_doc=data_doc)
#
# # 用新模型预测
