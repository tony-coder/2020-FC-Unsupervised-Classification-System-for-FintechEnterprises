...
#         max_level=10
#
#     labels = np.ceil(labels / model.n_clusters * max_level)  # scatter the value between 1-5
#
...
def predict(data_matrix, model, ascending_order=True, default_value=0):  # 只能用于预测打分
    ...
    labels = np.ceil(labels / model.n_clusters * max_level)  # scatter the value between 1-max_level
...
def get_trans_label(model, labels, default_value=0):
    ...
    d = dict(zip(centers, list(range(len(centers)))))  # number the center of model
    d = dict(sorted(d.items(), key=lambda x: x[0]))  # sort the dictionary as the ascending order of its values
